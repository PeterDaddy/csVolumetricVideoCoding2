        ...
        for k = 1:plane
            for i = 1:size(frame,1)/simulation_parameter.macro_block_size
                for j = 1:size(frame,2)/simulation_parameter.macro_block_size
                   ...
                   entropy_bpp_uncompressed = entropy_bpp_uncompressed + (Measurement_Entropy(y.measurement{i,j,k},(size(frame,1)*size(frame,2))));
                end
            end
        end
                ...
                for i = 1:size(sub_frame_temp,1)
                    for j = 1:size(sub_frame_temp,2)
                        ...
                        if(frame_number == 1)
                            ...
                            qp                                      = 64;
                            q_mtx                                   = [2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1); 
                                                                       2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1); 
                                                                       2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1);
                                                                       2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1) 2^(log2(qp)-1)];
                            ...
                            coded_sub_frame{i,j,k,z}                = dequantization_coded_sub_frame{i,j,k,z} + prediction_candidate;
                            %__Next iteration__
                            %___Big Bug___it should not be cell but matrix
                            %in set of z
                            intra_buffer_encoder{i,j,k}             = coded_sub_frame{i,j,k,z} + prediction_candidate;
                        else
